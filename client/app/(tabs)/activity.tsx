...
const DUMMY_TICKETS: Ticket[] = [
  { id: '1', spot: 'Pemancingan Galatama', date: '12 Jan 2024', status: 'active', price: 50000 },
  { id: '2', spot: 'Danau Toba Spot', date: '01 Jan 2024', status: 'history', price: 75000 },
  { id: '3', spot: 'Laut Selatan', date: '20 Des 2023', status: 'history', price: 100000 },
];
 ...
 */
const FilterTabs = ({ currentTab, onTabChange }: { currentTab: TabType, onTabChange: (t: TabType) => void }) => (
  <View style={styles.tabContainer}>
    <TouchableOpacity 
      style={[styles.tabButton, currentTab === TAB_OPTIONS.ACTIVE && styles.activeTab]} 
      onPress={() => onTabChange(TAB_OPTIONS.ACTIVE)}
      activeOpacity={0.7}
    >
      <Text style={[styles.tabText, currentTab === TAB_OPTIONS.ACTIVE && styles.activeTabText]}>
        Tiket Aktif
      </Text>
    </TouchableOpacity>

    <TouchableOpacity 
      style={[styles.tabButton, currentTab === TAB_OPTIONS.HISTORY && styles.activeTab]} 
      onPress={() => onTabChange(TAB_OPTIONS.HISTORY)}
      activeOpacity={0.7}
    >
      <Text style={[styles.tabText, currentTab === TAB_OPTIONS.HISTORY && styles.activeTabText]}>
        Riwayat
      </Text>
    </TouchableOpacity>
  </View>
...
const TicketCard = ({ item }: { item: Ticket }) => {
  ...
  const getStatusStyle = (status: TabType) => {
    return status === TAB_OPTIONS.ACTIVE 
      ? { bg: COLORS.successBg, text: COLORS.successText, label: 'Segera' }
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardBody}>
        <View style={styles.row}>
            <Ionicons name="calendar-outline" size={16} color={COLORS.textMuted} style={{marginRight: 5}}/>
